/* Compare three-row Q8 reuse against the deployed pair/six-row paths at all production shapes.
 ...
 for(unsigned j=0;j<sizeof(shapes)/sizeof(*shapes);j++) {
  ...
  for(unsigned r=0;r<sizeof(rows)/sizeof(*rows);r++) {
   for(unsigned arm=0;arm<2;arm++) {
    CHECK(ds41_levers_set("mtp_q8_trio_mask",arm?127:0));CHECK(ds4_gpu_begin_commands());
    CHECK(ds4_gpu_matmul_q8_0_decode_rows_exact_tensor(out,map,size,256,k,n,xt,rows[r]));
