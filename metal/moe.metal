...
// Two target rows, same grouped Q8 scalar arithmetic, shared weights.
template<bool ROUND>
static inline void dsv41_attn_low_rows2_body(
        constant ds4_metal_args_mul_mv_id & args,
        device const char * src0s,
        device const char * src1,
        device       char * dst,
    ...
    dsv41_q8_rows2<thread ds4_metal_args_mul_mv &, ROUND>(
        args0,
        src0_cur,
        src1_cur,
        dst_cur,
...
template<bool ROUND>
static inline void dsv41_attn_low_mma6_body(
        constant ds4_metal_args_mul_mv_id & args,
        device const char * src0s,
        device const char * src1,
        device       char * dst,
    ...
    dsv41_q8_mma6<thread ds4_metal_args_mul_mv &, ROUND>(
        args0,
        src0_cur,
        src1_cur,
        dst_cur,
...
kernel void kernel_dsv41_attn_low_mma6(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_attn_low_mma6_body<false>(args,w,x,out,shmem,tg,lane,sg);
}
kernel void kernel_dsv41_attn_low_mma6_bf16(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_attn_low_mma6_body<true>(args,w,x,out,shmem,tg,lane,sg);
}

template<bool ROUND>
static inline void dsv41_attn_low_rows2_f16acc_body(
        constant ds4_metal_args_mul_mv_id & args,
        device const char * src0s,
        device const char * src1,
        device       char * dst,
    ...
    dsv41_q8_rows2_f16acc<thread ds4_metal_args_mul_mv &, ROUND>(
        args0,
        src0_cur,
        src1_cur,
        dst_cur,
...
template<bool ROUND>
static inline void dsv41_attn_low_stream6_body(
        constant ds4_metal_args_mul_mv_id & args,
        device const char * src0s,
        device const char * src1,
        device       char * dst,
    ...
    dsv41_q8_stream6<thread ds4_metal_args_mul_mv &, ROUND>(
        args0,
        src0_cur,
        src1_cur,
        dst_cur,
...
kernel void kernel_dsv41_attn_low_stream6(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    // Grid z is group only: input strides span the WHOLE token's head vector;
    // output strides span all groups' low ranks. The weight base stays group-local.
    dsv41_attn_low_stream6_body<false>(args,w,x,out,shmem,tg,lane,sg);
}

kernel void kernel_dsv41_attn_low_stream6_bf16(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
    ...
    // Grid z is group only: input strides span the WHOLE token's head vector;
    // output strides span all groups' low ranks. The weight base stays group-local.
    dsv41_attn_low_stream6_body<true>(args,w,x,out,shmem,tg,lane,sg);
}

kernel void kernel_dsv41_attn_low_rows2(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_attn_low_rows2_body<false>(args,w,x,out,shmem,tg,lane,sg);
}

kernel void kernel_dsv41_attn_low_rows2_bf16(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_attn_low_rows2_body<true>(args,w,x,out,shmem,tg,lane,sg);
}

kernel void kernel_dsv41_attn_low_pair6(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    // iid1 in the shared helper is a TOKEN index; each grid bank is a PAIR.
    tg.z = (2u*(tg.z/args.nei0))*args.nei0 + tg.z%args.nei0;
    dsv41_attn_low_rows2_body<false>(args,w,x,out,shmem,tg,lane,sg);
}

kernel void kernel_dsv41_attn_low_pair6_bf16(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
...
kernel void kernel_dsv41_attn_low_pair6_f16acc(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
    ...
    // iid1 in the shared helper is a TOKEN index; each grid bank is a PAIR.
    tg.z = (2u*(tg.z/args.nei0))*args.nei0 + tg.z%args.nei0;
    dsv41_attn_low_rows2_f16acc_body<false>(args,w,x,out,shmem,tg,lane,sg);
}

kernel void kernel_dsv41_attn_low_pair6_f16acc_bf16(
        constant ds4_metal_args_mul_mv_id &args, device const char *w,
        device const char *x, device char *out,
        threadgroup char *shmem [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]],
    ...
    // iid1 in the shared helper is a TOKEN index; each grid bank is a PAIR.
    tg.z = (2u*(tg.z/args.nei0))*args.nei0 + tg.z%args.nei0;
    dsv41_attn_low_rows2_f16acc_body<true>(args,w,x,out,shmem,tg,lane,sg);
}
...
// Speed-only variant: retain parent geometry, no planner or shared K panels.
// Products and scale/minimum correction are FP32; dot sums are FP16. Packed
// nibbles are shifted to 0..15 before conversion, avoiding artificial overflow.
kernel void kernel_dsv41_q4_gu_f16acc6(
        constant ds4_metal_args_mul_mv_id & args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args & act,
        device const char * src0_gate,
        device const char * src0_up,
    ...
    half sumg[N_R0_Q4_K] = {0.h};
    half sumu[N_R0_Q4_K] = {0.h};
    uint16_t sc16[4];
    thread const uint8_t *sc8 = (thread const uint8_t *)sc16;
    ...
    for (int ib = ix; ib < nb; ib += 4) {
        float yl[16];
        float yh[16];
        half4 sumy = half4(0.h);

        for (short i = 0; i < 8; ++i) {
            yl[i + 0] = y4[i +   0]; sumy[0] += half(yl[i + 0]);
            yl[i + 8] = y4[i +  32]; sumy[1] += half(yl[i + 8]);
            yh[i + 0] = y4[i + 128]; sumy[2] += half(yh[i + 0]);
            yh[i + 8] = y4[i + 160]; sumy[3] += half(yh[i + 8]);
        }
        ...
        for (short row = 0; row < N_R0_Q4_K; row++) {
            ...
            device const uint16_t *qg2 = qg1 + 32;
            half4 acc1g = half4(0.h);
            half4 acc2g = half4(0.h);

            FOR_UNROLL (short i = 0; i < 4; ++i) {
                acc1g[0] += half(yl[2 * i + 0] * float((qg1[i] & 0x000F)));
                acc1g[1] += half(yl[2 * i + 1] * float((qg1[i] & 0x0F00) >> 8));
                acc1g[2] += half(yl[2 * i + 8] * float((qg1[i] & 0x00F0) >> 4));
                acc1g[3] += half(yl[2 * i + 9] * float((qg1[i] & 0xF000) >> 12));
                acc2g[0] += half(yh[2 * i + 0] * float((qg2[i] & 0x000F)));
                acc2g[1] += half(yh[2 * i + 1] * float((qg2[i] & 0x0F00) >> 8));
                acc2g[2] += half(yh[2 * i + 8] * float((qg2[i] & 0x00F0) >> 4));
                acc2g[3] += half(yh[2 * i + 9] * float((qg2[i] & 0xF000) >> 12));
            }

            const float block_g = float(dhg[0]) * (
                (float(acc1g[0])+float(acc1g[1]))*float(sc8[0]) +
                (float(acc1g[2])+float(acc1g[3]))*float(sc8[1]) +
                (float(acc2g[0])+float(acc2g[1]))*float(sc8[4]) +
                (float(acc2g[2])+float(acc2g[3]))*float(sc8[5])) -
                float(dhg[1]) * (float(sumy[0])*float(sc8[2]) + float(sumy[1])*float(sc8[3]) +
                                  float(sumy[2])*float(sc8[6]) + float(sumy[3])*float(sc8[7]));
            sumg[row] += half(block_g);
            ...
            device const uint16_t *qu2 = qu1 + 32;
            half4 acc1u = half4(0.h);
            half4 acc2u = half4(0.h);

            FOR_UNROLL (short i = 0; i < 4; ++i) {
                acc1u[0] += half(yl[2 * i + 0] * float((qu1[i] & 0x000F)));
                acc1u[1] += half(yl[2 * i + 1] * float((qu1[i] & 0x0F00) >> 8));
                acc1u[2] += half(yl[2 * i + 8] * float((qu1[i] & 0x00F0) >> 4));
                acc1u[3] += half(yl[2 * i + 9] * float((qu1[i] & 0xF000) >> 12));
                acc2u[0] += half(yh[2 * i + 0] * float((qu2[i] & 0x000F)));
                acc2u[1] += half(yh[2 * i + 1] * float((qu2[i] & 0x0F00) >> 8));
                acc2u[2] += half(yh[2 * i + 8] * float((qu2[i] & 0x00F0) >> 4));
                acc2u[3] += half(yh[2 * i + 9] * float((qu2[i] & 0xF000) >> 12));
            }

            const float block_u = float(dhu[0]) * (
                (float(acc1u[0])+float(acc1u[1]))*float(sc8[0]) +
                (float(acc1u[2])+float(acc1u[3]))*float(sc8[1]) +
                (float(acc2u[0])+float(acc2u[1]))*float(sc8[4]) +
                (float(acc2u[2])+float(acc2u[3]))*float(sc8[5])) -
                float(dhu[1]) * (float(sumy[0])*float(sc8[2]) + float(sumy[1])*float(sc8[3]) +
                                  float(sumy[2])*float(sc8[6]) + float(sumy[3])*float(sc8[7]));
            sumu[row] += half(block_u);
    ...
    for (int row = 0; row < N_R0_Q4_K && first_row + row < args.ne0; ++row) {
        const float gate = simd_sum(float(sumg[row]));
        const float up = simd_sum(float(sumu[row]));
        if (tiisg == 0) {
            const uint out_row = first_row + row;
            float g = gate;
            float u = up;
...
// H2: compact route owners into multiplicity buckets, entirely on the GPU.
// indirect[b] is {channel tiles, 1, expert jobs}; job is {expert,count,route[6]}.
struct dsv41_gu_plan {
    uint indirect[6][3];
    int job[6][36][8];
};
static inline void dsv41_gu_plan_build(constant ds4_metal_args_mul_mv_id &args,
        device const char *ids, device dsv41_gu_plan *plan,
        uint mask, uint shared_columns, uint tid) {
    if (tid != 0) return;
    for (uint b=0;b<6;b++) {
        const uint columns=b ? shared_columns : 4u;
        plan->indirect[b][0]=(uint(args.ne01)+columns-1u)/columns;
        plan->indirect[b][1]=1;
        plan->indirect[b][2]=0;
    }
    const int n=args.nei0*args.nei1;
    for (int pair=0;pair<n;pair++) {
        int expert=((device const int *)(ids+(pair/args.nei0)*args.nbi1))[pair%args.nei0];
        bool owner=true;
        for (int j=0;j<pair;j++)
            if (((device const int *)(ids+(j/args.nei0)*args.nbi1))[j%args.nei0]==expert) owner=false;
        if (!owner) continue;
        int routes[36], count=0;
        for (int j=pair;j<n;j++)
            if (((device const int *)(ids+(j/args.nei0)*args.nbi1))[j%args.nei0]==expert) routes[count++]=j;
        if (count>6 || expert<0 || expert>=args.ne02 || !(mask & (1u<<(count-1)))) {
            for (int j=0;j<count;j++) {
                uint at=plan->indirect[0][2]++;
                plan->job[0][at][0]=expert; plan->job[0][at][1]=1;
                plan->job[0][at][2]=routes[j];
            }
        } else {
            uint at=plan->indirect[count-1][2]++;
            plan->job[count-1][at][0]=expert; plan->job[count-1][at][1]=count;
            for (int j=0;j<count;j++) plan->job[count-1][at][j+2]=routes[j];
        }
    }
}

kernel void kernel_dsv41_gu_plan(constant ds4_metal_args_mul_mv_id &args,
        device const char *ids, device dsv41_gu_plan *plan,
        constant uint &mask, uint tid [[thread_position_in_grid]]) {
    dsv41_gu_plan_build(args,ids,plan,mask,4u,tid);
}
kernel void kernel_dsv41_gu_mk6_plan(constant ds4_metal_args_mul_mv_id &args,
        device const char *ids, device dsv41_gu_plan *plan,
        constant uint &mask, uint tid [[thread_position_in_grid]]) {
    dsv41_gu_plan_build(args,ids,plan,mask,16u,tid);
}

static inline void dsv41_h2_gu_device_body(
        constant ds4_metal_args_mul_mv_id & args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args & act,
        device const char * src0_gate,
        device const char * src0_up,
...
static inline void dsv41_h2_gu_shared_body(
        constant ds4_metal_args_mul_mv_id & args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args & act,
        device const char * src0_gate,
        device const char * src0_up,
    ...
    const int32_t i02 = ((device const int32_t *)(ids + iid1 * args.nbi1))[idx];
    if (!ds4_tp_owns_expert(i02, args.ne02, args.tp_rank, args.tp_world)) return;
    (void)src0_gate; (void)src0_up;
    const int64_t i11 = idx % args.ne11;
    const int64_t i12 = iid1;

    threadgroup const char *src0_gate_cur = shmem;
    threadgroup const char *src0_up_cur = shmem + 4u*args.nb01;
    device const char *src1_cur      = src1      + i11 * args.nb11 + i12 * args.nb12;
    ...
    threadgroup const block_q4_K *xg =
        (threadgroup const block_q4_K *)(src0_gate_cur + (uint64_t)sgitg * N_R0_Q4_K * args.nb01);
    threadgroup const block_q4_K *xu =
        (threadgroup const block_q4_K *)(src0_up_cur + (uint64_t)sgitg * N_R0_Q4_K * args.nb01);
    device const float *y = (device const float *)src1_cur;
    device const float *y4 = y + ix * QK_K + 64 * iq + 8 * ir;
    ...
    for (int ib = ix; ib < nb; ib += 4) {
        ...
        threadgroup const uint16_t *scg = (threadgroup const uint16_t *)xg[ib].scales + iq;
        threadgroup const uint16_t *qg1 = (threadgroup const uint16_t *)xg[ib].qs + 16 * iq + 4 * ir;
        threadgroup const half *dhg = &xg[ib].d;
        threadgroup const uint16_t *scu = (threadgroup const uint16_t *)xu[ib].scales + iq;
        threadgroup const uint16_t *qu1 = (threadgroup const uint16_t *)xu[ib].qs + 16 * iq + 4 * ir;
        threadgroup const half *dhu = &xu[ib].d;
        ...
        for (short row = 0; row < N_R0_Q4_K; row++) {
            ...
            threadgroup const uint16_t *qg2 = qg1 + 32;
            float4 acc1g = {0.f, 0.f, 0.f, 0.f};
            float4 acc2g = {0.f, 0.f, 0.f, 0.f};
            ...
            threadgroup const uint16_t *qu2 = qu1 + 32;
            float4 acc1u = {0.f, 0.f, 0.f, 0.f};
            float4 acc2u = {0.f, 0.f, 0.f, 0.f};
...
// Each row retains one original pair of SIMD groups. Weight bytes are copied,
// not dequantized, into one shared four-output-channel tile. No per-thread row
// loop or half2 scale rewrite from the failed B kernel enters its arithmetic.
template<ushort NT>
static inline void dsv41_h2_gu(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        device const dsv41_gu_plan *plan, threadgroup char *cache,
        uint3 tg, ushort tid, ushort lane, ushort sg) {
    device const int *job=plan->job[NT-1][tg.z];
    if (NT>1) {
        const uint tile_bytes=4u*uint(args.nb01), words=tile_bytes/4u;
        const uint64_t at=uint64_t(job[0])*args.nb02+uint64_t(tg.x)*tile_bytes;
        device const uint *g=(device const uint *)(gate+at);
        device const uint *u=(device const uint *)(up+at);
        threadgroup uint *dstg=(threadgroup uint *)cache;
        threadgroup uint *dstu=(threadgroup uint *)(cache+tile_bytes);
        for (uint i=tid;i<words;i+=NT*64u) { dstg[i]=g[i]; dstu[i]=u[i]; }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
    const ushort cohort=sg/2u, local_sg=sg%2u;
    tg.z=uint(job[2+cohort]);
    if (NT==1)
        dsv41_h2_gu_device_body(args,act,gate,up,x,outg,outu,mid,ids,weights,cache,tg,tid,lane,local_sg);
    else
        dsv41_h2_gu_shared_body(args,act,gate,up,x,outg,outu,mid,ids,weights,cache,tg,tid,lane,local_sg);
}

kernel void kernel_dsv41_h2_gu_1(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        device const dsv41_gu_plan *plan, threadgroup char *cache [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]], ushort tid [[thread_index_in_threadgroup]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_h2_gu<1>(args,act,gate,up,x,outg,outu,mid,ids,weights,plan,cache,tg,tid,lane,sg);
}

kernel void kernel_dsv41_h2_gu_2(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        uint3 tg [[threadgroup_position_in_grid]], ushort tid [[thread_index_in_threadgroup]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_h2_gu<2>(args,act,gate,up,x,outg,outu,mid,ids,weights,plan,cache,tg,tid,lane,sg);
}

kernel void kernel_dsv41_h2_gu_3(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        uint3 tg [[threadgroup_position_in_grid]], ushort tid [[thread_index_in_threadgroup]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_h2_gu<3>(args,act,gate,up,x,outg,outu,mid,ids,weights,plan,cache,tg,tid,lane,sg);
}

kernel void kernel_dsv41_h2_gu_4(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        uint3 tg [[threadgroup_position_in_grid]], ushort tid [[thread_index_in_threadgroup]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_h2_gu<4>(args,act,gate,up,x,outg,outu,mid,ids,weights,plan,cache,tg,tid,lane,sg);
}

kernel void kernel_dsv41_h2_gu_5(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        uint3 tg [[threadgroup_position_in_grid]], ushort tid [[thread_index_in_threadgroup]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_h2_gu<5>(args,act,gate,up,x,outg,outu,mid,ids,weights,plan,cache,tg,tid,lane,sg);
}

kernel void kernel_dsv41_h2_gu_6(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        uint3 tg [[threadgroup_position_in_grid]], ushort tid [[thread_index_in_threadgroup]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    dsv41_h2_gu<6>(args,act,gate,up,x,outg,outu,mid,ids,weights,plan,cache,tg,tid,lane,sg);
}

// Tier 2: FP32 Q4_K panels and a fixed lane-residue K tree.
// No parent arithmetic identity is claimed. All multiplicities (including the
// scalar replay oracle) use these same explicit per-element operations.
// Limit contraction to this one multiply-add expression. On the offline
// toolchain this emits llvm.fmuladd without fast/reassoc flags; the standard
// fma wrappers emit fast-flagged calls even inside a locally safe scope.
static inline float dsv41_mk6_madd(float a, float b, float c) {
#pragma METAL fp math_mode(safe)
#pragma METAL fp contract(on)
    return a*b+c;
}
static inline float dsv41_mk6_q4(device const block_q4_K *row, uint k) {
#pragma METAL fp math_mode(safe)
#pragma METAL fp contract(off)
    device const block_q4_K *block=row+k/256u;
    const uint within=k%256u, group=within/32u, lane=within%32u;
    const uchar2 sm=get_scale_min_k4_just2(int(group),0,block->scales);
    const uint q=(block->qs[(group/2u)*32u+lane] >> (4u*(group%2u))) & 15u;
    const float scale=float(block->d)*float(sm.x);
    const float minimum=float(block->dmin)*float(sm.y);
    return scale*float(q)-minimum;
}
static inline float dsv41_mk6_reduce(float value) {
#pragma METAL fp math_mode(safe)
#pragma METAL fp contract(off)
    // Explicit tree, shared by NT=1..6; only lane zero stores the result.
    value=value+simd_shuffle_xor(value,16);
    value=value+simd_shuffle_xor(value,8);
    value=value+simd_shuffle_xor(value,4);
    value=value+simd_shuffle_xor(value,2);
    value=value+simd_shuffle_xor(value,1);
    return value;
}
static inline float dsv41_mk6_mid(float gate, float up, float weight, float c) {
#pragma METAL fp math_mode(safe)
#pragma METAL fp contract(off)
    if (c>1.0e-6f) { gate=min(gate,c); up=clamp(up,-c,c); }
    const float silu=gate/(1.0f+precise::exp(-gate));
    return (silu*up)*weight;
}
static inline void dsv41_mk6_store(
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device char *outg, device char *outu, device char *mid,
        device const char *weights, uint pair, uint column, ushort lane,
        float gate_partial, float up_partial) {
#pragma METAL fp math_mode(safe)
#pragma METAL fp contract(off)
    const float g=dsv41_mk6_reduce(gate_partial), u=dsv41_mk6_reduce(up_partial);
    if (lane==0) {
        const float weight=*(device const float *)(weights+uint64_t(pair)*act.weight_stride);
        ((device float *)(outg+uint64_t(pair)*act.gate_row_stride))[column]=g;
        ((device float *)(outu+uint64_t(pair)*act.up_row_stride))[column]=u;
        ((device float *)(mid+uint64_t(pair)*act.mid_row_stride))[column]=
            dsv41_mk6_mid(g,u,weight,act.clamp_value);
    }
}
template<ushort NT>
static inline void dsv41_gu_mk6(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *weights, device const dsv41_gu_plan *plan,
        threadgroup float *panel, uint3 tg, ushort tid, ushort lane, ushort sg) {
#pragma METAL fp math_mode(safe)
#pragma METAL fp contract(off)
    constexpr uint N=NT==1 ? 4u : 16u;
    constexpr uint THREADS=N*16u;
    device const int *job=plan->job[NT-1][tg.z];
    const int expert=job[0];
    // A uniform early return keeps every threadgroup barrier well formed.
    // Valid router IDs are a precondition; never read an invalid weight address.
    if (expert<0 || expert>=args.ne02) return;
    threadgroup float *pg=panel, *pu=panel+N*32u, *px=panel+2u*N*32u;
    const uint col=tg.x*N;
    // Named accumulators avoid dynamically indexed thread arrays in AIR.
    // Compile-time NT conditions remove inactive rows, leaving 4*NT floats.
    float2 ag0=0, ag1=0, ag2=0, ag3=0, ag4=0, ag5=0;
    float2 au0=0, au1=0, au2=0, au3=0, au4=0, au5=0;
    for (uint k0=0;k0<uint(args.ne00);k0+=32u) {
        for (uint i=tid;i<N*32u;i+=THREADS) {
            const uint c=col+i/32u, k=k0+i%32u;
            device const block_q4_K *g=(device const block_q4_K *)(
                gate+uint64_t(expert)*args.nb02+uint64_t(c)*args.nb01);
            device const block_q4_K *u=(device const block_q4_K *)(
                up+uint64_t(expert)*args.nb02+uint64_t(c)*args.nb01);
            // Dispatch is restricted to exact N/K panels by the host.
            pg[i]=dsv41_mk6_q4(g,k); pu[i]=dsv41_mk6_q4(u,k);
        }
        for (uint i=tid;i<NT*32u;i+=THREADS) {
            const uint r=i/32u, pair=uint(job[2+r]);
            const uint token=pair/uint(args.nei0), slot=pair%uint(args.nei0);
            device const float *row=(device const float *)(x+
                uint64_t(slot%uint(args.ne11))*args.nb11+uint64_t(token)*args.nb12);
            px[i]=row[k0+i%32u];
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        const uint at=(uint(sg)*2u)*32u+lane;
        const float g0=pg[at], g1=pg[at+32u], u0=pu[at], u1=pu[at+32u];
#define DS41_MK6_ACC(R) if (NT > R) { \
            const float value=px[(R)*32u+lane]; \
            ag##R.x=dsv41_mk6_madd(g0,value,ag##R.x); ag##R.y=dsv41_mk6_madd(g1,value,ag##R.y); \
            au##R.x=dsv41_mk6_madd(u0,value,au##R.x); au##R.y=dsv41_mk6_madd(u1,value,au##R.y); \
        }
        DS41_MK6_ACC(0) DS41_MK6_ACC(1) DS41_MK6_ACC(2)
        DS41_MK6_ACC(3) DS41_MK6_ACC(4) DS41_MK6_ACC(5)
#undef DS41_MK6_ACC
        // All readers finish before any lane overwrites the one panel buffer.
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
#define DS41_MK6_STORE(R) if (NT > R) { \
        const uint pair=uint(job[2+(R)]), column=col+uint(sg)*2u; \
        dsv41_mk6_store(act,outg,outu,mid,weights,pair,column,lane,ag##R.x,au##R.x); \
        dsv41_mk6_store(act,outg,outu,mid,weights,pair,column+1u,lane,ag##R.y,au##R.y); \
    }
    DS41_MK6_STORE(0) DS41_MK6_STORE(1) DS41_MK6_STORE(2)
    DS41_MK6_STORE(3) DS41_MK6_STORE(4) DS41_MK6_STORE(5)
#undef DS41_MK6_STORE
}

kernel void kernel_dsv41_gu_mk6_1(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        device const dsv41_gu_plan *plan, threadgroup float *panel [[threadgroup(0)]],
        uint3 tg [[threadgroup_position_in_grid]], ushort tid [[thread_index_in_threadgroup]],
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    (void)ids;
    dsv41_gu_mk6<1>(args,act,gate,up,x,outg,outu,mid,weights,plan,panel,tg,tid,lane,sg);
}

kernel void kernel_dsv41_gu_mk6_2(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    (void)ids;
    dsv41_gu_mk6<2>(args,act,gate,up,x,outg,outu,mid,weights,plan,panel,tg,tid,lane,sg);
}

kernel void kernel_dsv41_gu_mk6_3(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    (void)ids;
    dsv41_gu_mk6<3>(args,act,gate,up,x,outg,outu,mid,weights,plan,panel,tg,tid,lane,sg);
}

kernel void kernel_dsv41_gu_mk6_4(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    (void)ids;
    dsv41_gu_mk6<4>(args,act,gate,up,x,outg,outu,mid,weights,plan,panel,tg,tid,lane,sg);
}

kernel void kernel_dsv41_gu_mk6_5(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    (void)ids;
    dsv41_gu_mk6<5>(args,act,gate,up,x,outg,outu,mid,weights,plan,panel,tg,tid,lane,sg);
}

kernel void kernel_dsv41_gu_mk6_6(constant ds4_metal_args_mul_mv_id &args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args &act,
        device const char *gate, device const char *up, device const char *x,
        device char *outg, device char *outu, device char *mid,
        device const char *ids, device const char *weights,
        ...
        ushort lane [[thread_index_in_simdgroup]], ushort sg [[simdgroup_index_in_threadgroup]]) {
    (void)ids;
    dsv41_gu_mk6<6>(args,act,gate,up,x,outg,outu,mid,weights,plan,panel,tg,tid,lane,sg);
}

// Exact-intent gate/up union for TWO target rows only.
kernel void kernel_dsv41_q4_gu_union2(
        constant ds4_metal_args_mul_mv_id & args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args & act,
        device const char * src0_gate,
        device const char * src0_up,
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    // One work item per original route. A row-0 owner also computes its
    // matching row-1 route; that duplicate row-1 work item exits uniformly.
    const int pair = tgpig.z, token = pair / args.nei0, slot = pair % args.nei0;
    device const int32_t *ids0 = (device const int32_t *)ids;
    device const int32_t *ids1 = (device const int32_t *)(ids + args.nbi1);
    const int expert = token ? ids1[slot] : ids0[slot];
    if (expert < 0 || expert >= args.ne02) return;
    int other = -1;
    for (int j = 0; j < args.nei0; ++j) {
        if (token && ids0[j] == expert) return;
        if (!token && ids1[j] == expert) other = args.nei0 + j;
    }
    const int nt = other < 0 ? 1 : 2;
    const int pairs[2] = {pair, other};
    const short NSG = FC_mul_mv_nsg;
    constexpr uint16_t kmask1 = 0x3f3f, kmask2 = 0x0f0f, kmask3 = 0xc0c0;
    const short ix = tiisg / 8, it = tiisg % 8, iq = it / 4, ir = it % 4;
    const int nb = args.ne00 / QK_K;
    const int first_row = (tgpig.x * NSG + sgitg) * N_R0_Q4_K;
    device const block_q4_K *xg = (device const block_q4_K *)(src0_gate +
        (uint64_t)expert * args.nb02 + (uint64_t)first_row * args.nb01);
    device const block_q4_K *xu = (device const block_q4_K *)(src0_up +
        (uint64_t)expert * args.nb02 + (uint64_t)first_row * args.nb01);
    device const float *y4[2];
    for (int t = 0; t < nt; ++t) {
        const int tok = pairs[t]/args.nei0, idx = pairs[t]%args.nei0;
        y4[t] = (device const float *)(src1 + (idx%args.ne11)*args.nb11 + tok*args.nb12)
            + ix*QK_K + 64*iq + 8*ir;
    }
    float sumg[2][N_R0_Q4_K] = {{0.f}}, sumu[2][N_R0_Q4_K] = {{0.f}};
    uint16_t sc16[4];
    thread const uint8_t *sc8 = (thread const uint8_t *)sc16;
    for (int ib = ix; ib < nb; ib += 4) {
        float yl[2][16], yh[2][16];
        float4 sumy[2] = {float4(0.f), float4(0.f)};
        for (int t = 0; t < nt; ++t) {
            for (short i = 0; i < 8; ++i) {
                yl[t][i+0] = y4[t][i+0];   sumy[t][0] += yl[t][i+0];
                yl[t][i+8] = y4[t][i+32];  sumy[t][1] += yl[t][i+8];
                yh[t][i+0] = y4[t][i+128]; sumy[t][2] += yh[t][i+0];
                yh[t][i+8] = y4[t][i+160]; sumy[t][3] += yh[t][i+8];
            }
        }
        device const uint16_t *scg = (device const uint16_t *)xg[ib].scales + iq;
        device const uint16_t *qg1 = (device const uint16_t *)xg[ib].qs + 16 * iq + 4 * ir;
        ...
        for (short row = 0; row < N_R0_Q4_K; row++) {
            ...
            const half2 dg = half2(dhg[0], dhg[1]);
            ushort qga[4], qgb[4];
            FOR_UNROLL (short i = 0; i < 4; ++i) {
                qga[i] = qg1[i]; qgb[i] = qg1[32+i];
            }
            for (int t = 0; t < nt; ++t) {
                float4 acc1g = {0.f, 0.f, 0.f, 0.f};
                float4 acc2g = {0.f, 0.f, 0.f, 0.f};
                ...
                FOR_UNROLL (short i = 0; i < 4; ++i) {
                    acc1g[0] += yl[t][2 * i + 0] * (qga[i] & 0x000F);
                    acc1g[1] += yl[t][2 * i + 1] * (qga[i] & 0x0F00);
                    acc1g[2] += yl[t][2 * i + 8] * (qga[i] & 0x00F0);
                    acc1g[3] += yl[t][2 * i + 9] * (qga[i] & 0xF000);
                    acc2g[0] += yh[t][2 * i + 0] * (qgb[i] & 0x000F);
                    acc2g[1] += yh[t][2 * i + 1] * (qgb[i] & 0x0F00);
                    acc2g[2] += yh[t][2 * i + 8] * (qgb[i] & 0x00F0);
                    acc2g[3] += yh[t][2 * i + 9] * (qgb[i] & 0xF000);
                }

                sumg[t][row] += dg[0] * ((acc1g[0] + 1.f / 256.f * acc1g[1]) * sc8[0] +
                                       (acc1g[2] + 1.f / 256.f * acc1g[3]) * sc8[1] * 1.f / 16.f +
                                       (acc2g[0] + 1.f / 256.f * acc2g[1]) * sc8[4] +
                                       (acc2g[2] + 1.f / 256.f * acc2g[3]) * sc8[5] * 1.f / 16.f) -
                             dg[1] * (sumy[t][0] * sc8[2] + sumy[t][1] * sc8[3] +
                                       sumy[t][2] * sc8[6] + sumy[t][3] * sc8[7]);
            ...
            const half2 du = half2(dhu[0], dhu[1]);
            ushort qua[4], qub[4];
            FOR_UNROLL (short i = 0; i < 4; ++i) {
                qua[i] = qu1[i]; qub[i] = qu1[32+i];
            }
            for (int t = 0; t < nt; ++t) {
                float4 acc1u = {0.f, 0.f, 0.f, 0.f};
                float4 acc2u = {0.f, 0.f, 0.f, 0.f};
                ...
                FOR_UNROLL (short i = 0; i < 4; ++i) {
                    acc1u[0] += yl[t][2 * i + 0] * (qua[i] & 0x000F);
                    acc1u[1] += yl[t][2 * i + 1] * (qua[i] & 0x0F00);
                    acc1u[2] += yl[t][2 * i + 8] * (qua[i] & 0x00F0);
                    acc1u[3] += yl[t][2 * i + 9] * (qua[i] & 0xF000);
                    acc2u[0] += yh[t][2 * i + 0] * (qub[i] & 0x000F);
                    acc2u[1] += yh[t][2 * i + 1] * (qub[i] & 0x0F00);
                    acc2u[2] += yh[t][2 * i + 8] * (qub[i] & 0x00F0);
                    acc2u[3] += yh[t][2 * i + 9] * (qub[i] & 0xF000);
                }

                sumu[t][row] += du[0] * ((acc1u[0] + 1.f / 256.f * acc1u[1]) * sc8[0] +
                                       (acc1u[2] + 1.f / 256.f * acc1u[3]) * sc8[1] * 1.f / 16.f +
                                       (acc2u[0] + 1.f / 256.f * acc2u[1]) * sc8[4] +
                                       (acc2u[2] + 1.f / 256.f * acc2u[3]) * sc8[5] * 1.f / 16.f) -
                             du[1] * (sumy[t][0] * sc8[2] + sumy[t][1] * sc8[3] +
                                       sumy[t][2] * sc8[6] + sumy[t][3] * sc8[7]);
        ...
        for (int t = 0; t < nt; ++t) y4[t] += 4*QK_K;
    }
    for (int t = 0; t < nt; ++t) {
        const uint64_t pair_row = pairs[t];
        device float *gate_f32 = (device float *)dst_gate + pair_row*args.ne0;
        device float *up_f32 = (device float *)dst_up + pair_row*args.ne0;
        device float *mid_f32 = (device float *)(dst_mid + pair_row*act.mid_row_stride);
        const float route_weight = *(device const float *)(weights + pair_row*act.weight_stride);
        const float c = act.clamp_value;
        for (int row = 0; row < N_R0_Q4_K && first_row + row < args.ne0; ++row) {
            const float gate = simd_sum(sumg[t][row]);
            const float up = simd_sum(sumu[t][row]);
            if (tiisg == 0) {
                const uint out_row = first_row + row;
                float g = gate;
                float u = up;
...
/* Decode pass 2, D1: the same kernel with one output
 * row per simdgroup instead of two.  Every row's K reduction, its six-slot
 * serial accumulation into one `sumf`, its block loop and its final simd_sum
 * are textually the ones it had; only which simdgroup owns the row, and hence
 * the grid width (2,560 groups instead of 1,280), changes.  This is NOT R4:
 * the accumulator is never reset per expert and no partial is reduced later. */
...
// Compact prefill layout retiled to M8/N32/K32, NSG2.
// FP16 operands, FP32 MMA accumulators; original route-slot F32 mid for sum6.
// At six tokens each expert has <=6 distinct routed rows; map tiles begin at0.
template<typename block_q, short nl, void (*dequantize_func)(device const block_q *, short, thread half4x4 &)>
kernel void kernel_dsv41_gu_mma6_impl(
        constant ds4_metal_args_mul_mm_id & args,
        constant ds4_metal_dsv4_moe_swiglu_weight_args & act,
        device const char * src0_gate,
        device const char * src0_up,
    ...
    simdgroup_half8x8 ma_g[2];
    simdgroup_half8x8 ma_u[2];
    simdgroup_half8x8 mb[1];

    simdgroup_float8x8 mc_gate[2];
    simdgroup_float8x8 mc_up[2];
    ...
    for (int loop_k = 0; loop_k < args.ne00; loop_k += NK) {
        ...
        // The first SIMD group stages eight rows. Missing expert rows are zero.
        if (tiitg < 32) {
            const short ib = tiitg % NL1;
            const half2x4 values = row_b0 < nr1 ?
                half2x4(*((device const float2x4 *)y_b0)) : half2x4(0);
            *(threadgroup half2x4 *)(sb + 64*ib + 8*row_b0) = values;
        }
        ...
        threadgroup const half * lsma_gate = sa_gate + 2*64*sgitg;
        threadgroup const half * lsma_up   = sa_up + 2*64*sgitg;
    ...
    threadgroup float * temp_gate = (threadgroup float *) shmem;
    threadgroup float * temp_up = temp_gate + NR0*NR1;
    threadgroup float * temp_gate_str = temp_gate + 16*sgitg;
    threadgroup float * temp_up_str = temp_up + 16*sgitg;

    {
        for (short i = 0; i < 2; i++) {
            simdgroup_store(mc_gate[i], temp_gate_str + 8*i, NR0, 0, false);
            simdgroup_store(mc_up[i],   temp_up_str   + 8*i, NR0, 0, false);
        }
    }
...
typedef decltype(kernel_dsv41_gu_mma6_impl<block_q4_K, QK_NL, dequantize_q4_K>) dsv41_gu_mma6;
template [[host_name("kernel_dsv41_gu_mma6")]] kernel dsv41_gu_mma6 kernel_dsv41_gu_mma6_impl<block_q4_K, QK_NL, dequantize_q4_K>;
...
// V4.1 prefill bundle item (i): the same CULL_TAIL_SIMDGROUPS mechanism for
// Q4_K.  384 experts with 6 routes per token leave most live 32-row tiles
// half empty at every prefill chunk width, and simdgroups 2/3 then run a
// full MMA tree over rows that do not exist.  Staging, barriers and the
// epilogue are untouched; only the MMA of the idle row-half is skipped, so
// every surviving row keeps its own accumulation order and route order.
...
typedef decltype(kernel_mul_mm_id<32, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4, true>) mul_mm_id_q4_K_f32_tail_cull;
...
template [[host_name("kernel_mul_mm_id_q4_K_f32_tail_cull")]] kernel mul_mm_id_q4_K_f32_tail_cull kernel_mul_mm_id<32, half, half4x4, simdgroup_half8x8, half, half2x4, simdgroup_half8x8, block_q4_K, QK_NL, dequantize_q4_K, float, float4x4, float, float2x4, true>;
